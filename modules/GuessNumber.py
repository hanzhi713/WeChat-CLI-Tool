...
class GuessNumber:
    ...
    def __init__(self, from_user, args):
        try:
            ...
            upper = int(args[1])
            if upper <= lower:
                itchat.send_msg("Upper bound must be greater than the lower bound!".format(lower, upper),
            ...
            itchat.send_msg("Illegal Arguments!\nTwo integer parameters are required: [lower] and [upper]",
                        toUserName=from_user)
            raise Exception
        ...
        itchat.send_msg("Guessing between {} and {}\nType /q to quit the game".format(lower, upper), toUserName=from_user)

    def msg_handler(self, msg):
        from_user = msg['FromUserName']
        if msg['Text'] == '/q':
            itchat.send_msg("Quitting the game", toUserName=from_user)
            self.finished = True
            self.send_separator(from_user)
            return True
        ...
        try:
            ...
            itchat.send_msg("Illegal Input!", toUserName=from_user)
            return False

        if guess < self.number:
            self.trials += 1
            itchat.send_msg("Too Small. Try again.", toUserName=from_user)
            return False
        elif guess > self.number:
            self.trials += 1
            itchat.send_msg("Too Big. Try again.", toUserName=from_user)
            return False
        else:
            self.trials += 1
            itchat.send_msg("Bingo! You have got it in {} trials!".format(self.trials), toUserName=from_user)
            self.send_separator(from_user)
            self.finished = True
            return True
